...
class CaptureSession:
    ...
    def observe(self, packet: VoiceCapturePacket) -> tuple[CaptureTransition, ...]:
                ...
                if self._healthy_recovery_packets >= self._recovery_packets_required:
                    transitions.append(
                        self._change(
                            ...
                            f"{self._healthy_recovery_packets} consecutive healthy packets",
                        )
                    )
                    self._healthy_recovery_packets = 0
        else:
